...
void aprinde_led() {
    GPIO_PinOutSet(LED_PORT, LED_PIN);  // Setează pinul LED pentru a aprinde LED-ul
}

// Funcție pentru stingerea LED-ului onboard
void stinge_led() {
    GPIO_PinOutClear(LED_PORT, LED_PIN);  // Resetează pinul LED pentru a stinge LED-ul
}
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_gatt_server_attribute_value_id :
      if (evt->data.evt_gatt_server_attribute_value.attribute == gattdb_LED_IO) {
          ...
          uint8_t valoare = evt->data.evt_gatt_server_attribute_value.value.data[0];
          app_log("LED_IO : %x\r\n",valoare);
          // Dacă valoarea primită este 0x01, aprindem LED-ul
          if (valoare == 0x01) {
              aprinde_led();
              printf("LED aprins!\n");
